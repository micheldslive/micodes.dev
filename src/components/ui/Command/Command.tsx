    ...
    <KBarResults
      ...
      onRender={({ item, active }) =>
        typeof item === 'string' ? (
          ...
          <ResultItem action={item as TAction} active={active} />
        )
      }
    />
  );
...
type ResultItemProps = ComponentProps<'div'> & {
  action: TAction;
  active: boolean;
};

const ResultItem = ({ action, active, ref }: ResultItemProps) => {
  if (active) {
      ...
      {!!action.shortcut?.length && (
        <div className="grid grid-flow-col gap-1" aria-hidden>
          {action.shortcut.map((shortcut) => (
            <kbd
              key={shortcut}
              className="bg-white/10 px-2 py-1 text-slate-800 uppercase dark:text-slate-50"
            >
              {shortcut}
            </kbd>
          ))}
